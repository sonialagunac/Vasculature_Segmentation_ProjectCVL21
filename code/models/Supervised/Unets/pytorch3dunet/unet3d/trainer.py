...
class UNet3DTrainer:
    ...
    def fit(self):
        ...
        for _ in range(self.num_epoch, self.max_num_epochs):
            # train for one epoch
            should_terminate = self.train(loss_list)

            if should_terminate:
                logger.info('Stopping criterion is satisfied. Finishing training')
                return
            loss_torch = torch.Tensor(loss_list)
            torch.save(loss_torch, "/home/slaguna/Documents/semproject/3dunet_DV_loss.pytorch")
            self.num_epoch += 1
        logger.info(f"Reached maximum number of epochs: {self.max_num_epochs}. Finishing training...")
        lst = np.array(loss_list)
        loss_torch=torch.Tensor(loss_list)
        torch.save(loss_torch, '/home/slaguna/Documents/semproject/3dunet_DV_loss.pytorch')
        np.save('/home/slaguna/Documents/semproject/3dunet_DV_loss.npy', lst)
    def train(self,loss_list):
        ...
        for t in self.loaders['train']:
            ...
            # print(input_np.shape, 'output shape ', target_np.shape, 'target shape ')
            ...
            if self.num_iterations % self.log_after_iters == 0:
                ...
                loss_torch = torch.Tensor(loss_list)
                torch.save(loss_torch, '/home/slaguna/Documents/semproject/3dunet_DV_loss_iters.pytorch')

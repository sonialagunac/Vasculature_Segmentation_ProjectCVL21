...
def smart_padding(img, data_shape, lables_shape, stride):
    ...
    print('dz ', dz, 'dy ', dy, 'dx ', dx, 'stride ', stride)
    ...
    print('eff data shape ', effective_data_shape, 'img shape at this point ', img.shape)
    if effective_data_shape[0] < img.shape[0]:
        print('Padding in condition 4 ')
        img = np.pad(img,
                     (
    ...
    print('eff lable_shape ', effective_lable_shape, 'lables shape ', lables_shape, 'data shape ', data_shape, 'img size before pad', img.shape)
    if effective_lable_shape[0] < img.shape[0]:
        print('Padding in condition 7 ')
        ...
        img = np.pad(img, ((0, 0),
                           ...
                           ((data_shape[2] - lables_shape[2]) // 2,
                            (data_shape[2] - lables_shape[2]) // 2 + (
                                    data_shape[2] - lables_shape[2]) % 2)),
                     mode='reflect')
    print('img before return', img.shape)
    return img
...
class Single_Image_Eval(Dataset):
    def __init__(self,
        ...
        try:
            ...
            im_95 = np.percentile(img, 95)
            img[img >= im_95] = im_95
            img = np.divide(img, im_95)
        except:
            img = []
...
class Directory_Image_Train(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # To Tensor
        data = torch.Tensor(v).unsqueeze(0)
        lables = torch.Tensor(lbl // np.amax(self.lbl)).long()
        return data, lables
